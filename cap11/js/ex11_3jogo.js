...
window.addEventListener("load", () => {
  // Verifica se ambos os itens estão no localStorage
  if (!localStorage.getItem("jogoPalavra") || !localStorage.getItem("jogoDica")) {
    ...
    return; // Adiciona um retorno para evitar a execução do restante
  }
  ...
  const trocarStatus = (num) => {
    if (num >= 0) imgStatus.src = `img/status${num}.jpg`; // Corrigido para não permitir índices negativos
  };
